...
namespace _PeopleWorks__JF2_PBMS_EOL_Tester_IL
{
	class BarcodePrint
	{
		...
		public int nBCDAllOffsetX = 0;
		public int nBCDAllOffsetY = 0;
		public int nBCDbcdOffsetX = 0;
		public int nBCDbcdOffsetY = 0;
		public int nBCDtextOffsetX = 0;
		public int nBCDtextOffsetY = 0;
		public int nBCDsize = 0;
		public int nBCDStringHeight = 0;
		public int nBCDStringWidth = 0;
						...
						{
							 {
                                ...
                                int XOffset = nBCDAllOffsetX + nBCDbcdOffsetX;
                                int YOffset = nBCDAllOffsetY + nBCDbcdOffsetY;

	

								if (theApp._ModelInfo.bUseQRCode)
                                {
                                    strLabelData += String.Format("^FO{0},{1}", (XOffset < 0) ? 0 : XOffset, (YOffset < 0) ? 0 : YOffset);
                                    strLabelData += String.Format("^BQN,2,{0}", nBCDsize);
                                    strLabelData += String.Format("^FDMA\\{0}{1}^FS", strManualModelInfo, strManualPrintBCD);

                                }
                                else
                                {
                                    strLabelData += String.Format("^FS^FO{0},{1}", (XOffset < 0) ? 0 : XOffset, (YOffset < 0) ? 0 : YOffset);
                                    strLabelData += String.Format("^BXN,{0},200", nBCDsize);
                                    strLabelData += String.Format("^FD{0}{1}", strManualModelInfo, strManualPrintBCD);

                                }


                                XOffset = nBCDAllOffsetX + nBCDtextOffsetX;
                                YOffset = nBCDAllOffsetY + nBCDtextOffsetY;


                                strLabelData += String.Format("^FS^FO{0},{1}", (XOffset < 0) ? 0 : XOffset, (YOffset < 0) ? 0 : YOffset);
                                strLabelData += String.Format("^A0,{0},{1}", nBCDStringHeight, nBCDStringWidth);
						...
						{
							{
								...
								if (theApp._ModelInfo.bUseQRCode)
								{
									strLabelData += String.Format("^FO{0},{1}", (XOffset < 0) ? 0 : XOffset, (YOffset < 0) ? 0 : YOffset);
									strLabelData += String.Format("^BQN,2,{0}", nBCDsize);
									strLabelData += String.Format("^FDMA\\{0}{1}^FS", strModelInfo, strPrintBCD);
