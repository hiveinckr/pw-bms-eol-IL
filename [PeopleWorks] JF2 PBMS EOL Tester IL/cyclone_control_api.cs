...
namespace _PeopleWorks__JF2_PBMS_EOL_Tester_IL
{
	public static class cyclone_control_api
	{
		...
		public const string setCurrentImageByUniqueId = "setCurrentImageByUniqueId";
		...
		public const string DLL_FILENAME = "D:\\[PeopleWorks] JF2 PBMS EOL Tester IL\\CycloneControlSDK.dll";

		/* The following imported methods have wrapper methods to properly handle theWWWW
		...
		[DllImport(DLL_FILENAME, CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, EntryPoint = "clearOverlayProgramData")]
		private static extern byte unmanaged_clearOverlayProgramData(UInt32 cycloneHandle);
		[DllImport(DLL_FILENAME, CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, EntryPoint = "specifyOverlayProgramData")]
		private static extern byte unmanaged_specifyOverlayProgramData(UInt32 cycloneHandle, UInt32 targetAddress, UInt32 dataLength, IntPtr unmanagedCharArrayPtr);
		...
		[DllImport(DLL_FILENAME, CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, EntryPoint = "uploadRange")]
		private static extern byte unmanaged_uploadRange(UInt32 cycloneHandle, UInt32 targetAddress, UInt32 dataLength, IntPtr uploadFileName);
		...
		public static bool clearOverlayProgramData(UInt32 cycloneHandle)
		{
			try
			{
				byte unmanagedBool = unmanaged_clearOverlayProgramData(cycloneHandle);
		...
		public static bool specifyOverlayProgramData(UInt32 cycloneHandle, UInt32 targetAddress, UInt32 dataLength, Byte[] managedByteArray)
		{
			...
			try
			{
				byte unmanagedBool = unmanaged_specifyOverlayProgramData(cycloneHandle, targetAddress, dataLength, unmanagedByteArrayPtr);
				if (unmanagedBool.Equals(0))
				{
					return false;
				}
		...
		public static bool uploadRange(UInt32 cycloneHandle, UInt32 targetAddress, UInt32 dataLength, string uploadFileName)
		{
			IntPtr aFilePtr = Marshal.StringToHGlobalAnsi(uploadFileName);
			try
			{
				byte unmanagedBool = unmanaged_uploadRange(cycloneHandle, targetAddress, dataLength, aFilePtr);
				if (unmanagedBool.Equals(0))
				{
					return false;
				}
